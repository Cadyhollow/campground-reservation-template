...
export default function DiscountsPage() {
  ...
  function openEditForm(discount: Discount) {
    ...
    setForm({
      ...
      discount_value: discount.discount_type === 'flat'
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      discount_value: form.discount_type === 'flat'
        ? Math.round(parseFloat(form.discount_value as string) * 100)
        : parseInt(form.discount_value as string),
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"
                placeholder={form.discount_type === 'percent' ? 'e.g. 10' : 'e.g. 20.00'}
                type="number"
          ...
          <div className="flex gap-3 mt-4">
            ...
            >
              {saving ? 'Saving...' : editingDiscount ? 'Save Changes' : 'Create Code'}
            </button>
            <button
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y divide-gray-50">
          {discounts.map((discount) => (
            <div key={discount.id} className="px-6 py-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className={`w-3 h-3 rounded-full ${discount.is_active ? 'bg-green-500' : 'bg-gray-300'}`} />
                <div>
