...
export default function SitesPage() {
  ...
  async function handleSave() {
    if (!form.site_number || !form.base_rate) {
      toast.error('Site number and nightly rate are required.')
      return
    }
    setSaving(true)
    const payload = {
  ...
  async function toggleAvailability(site: Site) {
    ...
    toast.success(`Site ${site.site_number} ${!site.is_available ? 'activated' : 'deactivated'}`)
    fetchSites()
  }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            {editingSite ? `Edit Site ${editingSite.site_number}` : 'Add New Site'}
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. 14, Cabin 1, Tent A"
            ...
            {form.site_type === 'rv_site' && (
              <>
                <div>
                  ...
                  <select
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"
                    value={form.amp_service}
                    onChange={e => setForm({ ...form, amp_service: e.target.value })}
                  >
                ...
                <div>
                  ...
                  <select
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"
                    value={form.hookups}
                    onChange={e => setForm({ ...form, hookups: e.target.value })}
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 40"
                    type="number"
                    value={form.max_rv_length}
                    onChange={e => setForm({ ...form, max_rv_length: e.target.value })}
                  />
                </div>
              </>
            )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y divide-gray-50">
          {sites.map((site) => (
            <div key={site.id} className="px-6 py-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className={`w-3 h-3 rounded-full ${site.is_available ? 'bg-green-500' : 'bg-gray-300'}`} />
                <div>
                  <p className="font-semibold text-gray-900">
                    {siteTypeLabel(site.site_type)} {site.site_number}
                  </p>
                  <p className="text-sm text-gray-500">
                    {ampLabel(site.amp_service)} · {hookupLabel(site.hookups)}
                    {site.max_rv_length ? ` · Max ${site.max_rv_length}ft` : ''}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-green-700 font-semibold">${(site.base_rate / 100).toFixed(2)}/night</span>
                <button
                  onClick={() => toggleAvailability(site)}
