...
// component that reads settings, fees, add-ons and the site's turnover and hands them in below.
// Everything else in the booking link — site number, rate, nights, guests — is still read here
// from useSearchParams, so both halves read the same URL.
export default function BookingForm({
  ...
  // role can read none of them — a camper's browser query would come back empty and the page
  ...
  // Security PR 7-1: the turnover scan moved to lib/book-server.ts. It read every
  // non-cancelled reservation on this site from the browser to decide whether the early/late
  // checkboxes are offered — the same two comparisons /api/payment makes before it will honour
  // them, so the answer belongs on the same side as the charge.
  ...
  async function checkDiscount() {
    ...
    // Security PR 7-1: this used to read the `discounts` table from the browser and apply the
    // four validity rules here. The filter reaching the wire was the code the camper typed, but
    // the TABLE was readable — so anyone with the page open could enumerate every code the park
    // had issued, with its value and remaining uses. /api/discount answers about one code and
    // returns a verdict, not a row, and runs the same checkDiscount() the charge path runs.
    ...
    const data = await res.json()
    if (!data.valid) { setDiscountError(data.error || 'Invalid or expired discount code.'); setCheckingDiscount(false); return }
  ...
  // disagree, so anything shown here the server would not stand behind fails loudly instead
  // of being charged. Sharing the function is what keeps the two in step — see
  ...
  // The card fee's share of a Pay in Full charge, for the disclosure line on that button.
  // handlePayment charges cashTotal + this, which is exactly `total` — the card-only fee is
  // already inside total, and cashTotal is total minus it. So unlike the deposit, Pay in
  // Full's AMOUNT was already truthful and is deliberately left alone; only the fee it
  // contains was never named. Same expression handlePayment uses for paymentType 'full'.
  const fullSurcharge = cashTotal > 0 ? Math.round(cashTotal * cardOnlyFeesTotal / cashTotal) : 0
